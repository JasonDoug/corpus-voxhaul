...
describe('End-to-End Pipeline Test with Real Scientific PDF', () => {
  ...
  describe('Complete Pipeline with Humorous Agent', () => {
    it('should process a real scientific PDF through the entire pipeline', async () => {
      ...
      expect(analyzeResponse.body.status).toBe('segmenting');
      ...
      expect(contentAfterAnalysis?.extractedContent).toBeDefined();
      
      const extractedContent = contentAfterAnalysis!.extractedContent!;
      
      // Verify text extraction (Requirement 2.1)
      expect(extractedContent.pages).toBeDefined();
      expect(extractedContent.pages.length).toBeGreaterThan(0);
      expect(extractedContent.pages[0].text.length).toBeGreaterThan(0);
      console.log(`✓ Extracted text from ${extractedContent.pages.length} pages`);
      console.log(`  First page text length: ${extractedContent.pages[0].text.length} characters`);
      
      // Verify multi-modal content detection (Requirement 1.4, 2.2, 2.3, 2.4, 2.5)
      console.log(`✓ Detected content elements:`);
      console.log(`  - Figures: ${extractedContent.figures?.length || 0}`);
      console.log(`  - Tables: ${extractedContent.tables?.length || 0}`);
      console.log(`  - Formulas: ${extractedContent.formulas?.length || 0}`);
      console.log(`  - Citations: ${extractedContent.citations?.length || 0}`);
      
      // If figures exist, verify descriptions (Requirement 2.2)
      if (extractedContent.figures && extractedContent.figures.length > 0) {
        const figure = extractedContent.figures[0];
        expect(figure.description).toBeDefined();
        expect(figure.description.length).toBeGreaterThan(0);
        console.log(`✓ Figure description generated: "${figure.description.substring(0, 50)}..."`);
      }
      
      // Step 4: Run Content Segmentation
      console.log('\n=== Step 4: Running Content Segmentation ===');
      
      const segmentResponse = await request(app)
        .post(`/api/segment/${jobId}`)
        .expect(200);
      
      expect(segmentResponse.body.status).toBe('generating_script');
      console.log(`✓ Segmentation complete. Status: ${segmentResponse.body.status}`);
      
      // Verify segmented content
      const contentAfterSegmentation = await getContent(jobId);
      expect(contentAfterSegmentation?.segmentedContent).toBeDefined();
      
      const segmentedContent = contentAfterSegmentation!.segmentedContent!;
